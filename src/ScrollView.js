...
export default class ScrollView extends Component {
  static propTypes = {
    ...
    decelerationRate: function(props, propName, componentName) {
      ...
      if (
        (typeof val !== "number" && !decelerationRate[val]) ||
    ...
    //  events
    ...Object.keys(Events).reduce((combined, key) => {
  ...
  constructor(props) {
    ...
    const clampedScale = new Animated.diffClamp(
      scale,
      props.minimumZoomScale,
      props.maximumZoomScale
    );
    ...
    translate.addListener(value => {
      ...
      if (this._runningAnimations.length > 0) {
        ...
        if (
          clampedTranslate.x !== this._translateValue.x ||
          clampedTranslate.y !== this._translateValue.y
        ) {
          ...
          Animated.parallel([
            ...
            Animated.spring(this._animatedValues.scrollIndicator, {
              toValue: this.getScrollIndicatorOffset(
    ...
    const scrollIndicatorOpacityValue =
      StyleSheet.flatten([
        props.horizontalIndicatorStyle,
        props.verticalIndicatorStyle,
  ...
  getDecelerationRate() {
    return typeof this.props.decelerationRate === "number"
      ? this.props.decelerationRate
      : decelerationRate[this.props.decelerationRate];
  }

  getContentInset(
    scale = this.getClampedScale(),
    contentInset = this.props.contentInset
  ) {
    const start = {
      key: isRTL ? "right" : "left",
      value: contentInset.start
        ? contentInset.start
        : isRTL
        ? contentInset.right
        : contentInset.left
  ...
  getClampedTranslate({ x, y } = this._translateValue, scale = undefined) {
    ...
    const centeredContent = {
      x: sizeCheck.x
        ? clampers.x +
          (dimensions.width - this.state.width * clampedScale) * 0.5
        : 0,
      y: sizeCheck.y
        ? clampers.y +
          (dimensions.height - this.state.height * clampedScale) * 0.5
        : 0
    };
    ...
    const clampedValues = {
      x: Math.min(
        clampers.x + contentInset.left,
        Math.max(complementaryClampers.x - overScroll.x - contentInset.right, x)
      ),
      y: Math.min(
        clampers.y + contentInset.top,
        Math.max(
          complementaryClampers.y - overScroll.y - contentInset.bottom,
          y
        )
      )
    };
  ...
  getScrollIndicatorOffset(
    scale = this.getClampedScale(),
    translations = this.getClampedTranslate()
  ) {
    const dimensions = Dimensions.get("window");
    const scrollOffset = this.getScrollOffset(scale, translations);
    const scrollOffsetMax = this.getScrollOffset(
      scale,
      this.getClampers(scale).complementaryClampers
    );
    const ratio = {
    ...
    return {
      x: Math.min(
        Math.max(0, offset.x),
        dimensions.width - this._indicatorLayout.width
      ),
      y: Math.min(
        Math.max(0, offset.y),
        dimensions.height - this._indicatorLayout.height
      )
    };
  }

  animateScrollIndicatorOpacity(toValue) {
    this._activeScrollIndicatorAnimation &&
      this._activeScrollIndicatorAnimation.stop();
    this._activeScrollIndicatorAnimation = Animated.timing(
      this._animatedValues.scrollIndicatorOpacity,
      { toValue, useNativeDriver: true }
    );
    this._activeScrollIndicatorAnimation.start(
      () => (this._activeScrollIndicatorAnimation = null)
    );
  }
  ...
  _loadPanResponder() {
    ...
    this._panResponder = PanResponder.create({
      onStartShouldSetPanResponder: (evt, gestureState) =>
        this.props.scrollEnabled || this.props.pinchGestureEnabled,
      onStartShouldSetPanResponderCapture: (evt, gestureState) => false,
      onMoveShouldSetPanResponder: (evt, gestureState) =>
        (this.props.scrollEnabled || this.props.pinchGestureEnabled) &&
        Math.hypot(gestureState.dx, gestureState.dy) > 10,
  ...
  _startShouldSetResponder(evt, gestureState) {
    this._grant = evt.nativeEvent;
    console.log("_startShouldSetResponder");
    return this.props.onStartShouldSetResponder(evt, gestureState);
  }
  ...
  _moveShouldSetResponder(evt, gestureState) {
    ...
    console.log("_moveShouldSetResponder");
    return this.props.onMoveShouldSetResponder(evt, gestureState);
  }
  ...
  _handleMove(evt) {
    ...
    if (touches.length === 2 && this.props.pinchGestureEnabled) {
      ...
      const clampedTranslations = this.getClampedTranslate(
        this._translateStart,
        scale
      );
      ...
      if (this.props.directionalLockEnabled) {
        if (!this._directionalLock) {
          this._directionalLock = {
            value:
              m < 0.5
                ? directionalLock.horizontal
                : m > 2
                ? directionalLock.vertical
                : null,
            ...clampedTranslations
          };
        }
  ...
  _handleRelease(evt, gestureState) {
    ...
    const didClamptranslate =
      this._translateValue.x !== clampedTranslate.x ||
      this._translateValue.y !== clampedTranslate.y;
    const didShrink =
      this.props.bounces ||
      (clampedScale < this._scaleStart && didClamptranslate && this._isZooming);
    const v = {
  ...
  getScrollOffset(
    scale = this.getClampedScale(),
    translation = this._translateValue
  ) {
  ...
  scrollTo({
    ...
    const translations =
      overScroll === true
        ? _contentOffset
        : this.getClampedTranslate(_contentOffset, clampedScale);
    ...
    if (animated) {
      this._activeScrollIndicatorAnimation &&
        this._activeScrollIndicatorAnimation.stop(); // may cause problems when running a bunch of animation in another place
      const animations = [
        Animated.timing(this._animatedValues.translate, {
          toValue: translations,
          useNativeDriver: true
        }),
        Animated.timing(this._animatedValues.scrollIndicator, {
          toValue: scrollIndicatorOffset,
          useNativeDriver: true
        })
      ];
      this.scrollResponderIsAnimating = true;
      ...
      if (scale)
        animations.unshift(
          Animated.timing(this._animatedValues.scale, {
            toValue: this.getClampedScale(scale),
            useNativeDriver: true
          })
        );
      Animated.parallel(animations).start(arg => {
        this._scrollToInProgress = false;
        this.scrollResponderIsAnimating = false;
      ...
      if (scale)
        this._animatedValues.scale.setValue(this.getClampedScale(scale));
  ...
  scrollResponderScrollNativeHandleToKeyboard(
    ...
    this._overScroll.y = preventNegativeScrollOffset
      ? Math.max(extraHeight, 0)
      : extraHeight;
  }
  ...
  flashScrollIndicators() {
    this._activeScrollIndicatorAnimation &&
      this._activeScrollIndicatorAnimation.stop();
    Animated.sequence([
      Animated.timing(this._animatedValues.scrollIndicatorOpacity, {
        toValue: 1,
        useNativeDriver: true
      }),
      Animated.timing(this._animatedValues.scrollIndicatorOpacity, {
  ...
  _eventSender(eventType) {
    ...
    if (
      eventType === Events.onMomentumScrollEnd &&
        ...
        {showsHorizontalScrollIndicator && (
          <Animated.View
            ...
            style={[
              ...
              indicatorStyle,
              horizontalIndicatorStyle,
              this.indicatorStyles.horizontal
            ]}
          />
        )}
        {showsVerticalScrollIndicator && (
